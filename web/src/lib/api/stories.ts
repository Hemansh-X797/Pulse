...
export async function createStory(mediaUrl: string, mediaType: 'image' | 'video' = 'image', durationSeconds?: number) {
  const { data: userData } = await supabase.auth.getUser();
  if (!userData.user) throw new Error('not authenticated');
  const { error } = await supabase
    .from('stories')
    .insert({ author_id: userData.user.id, media_url: mediaUrl, media_type: mediaType, duration_seconds: durationSeconds ?? null });
...
export async function listActiveStoryGroups(): Promise<StoryGroup[]> {
  const { data, error } = await supabase
    .from('stories')
    .select(
      'id, media_url, media_type, duration_seconds, created_at, expires_at, author:author_id(id, username, display_name, avatar_url, accent_color_top, accent_color_bottom)'
    )
  ...
  for (const row of data ?? []) {
    ...
    groups.get(author.id)!.stories.push({
      id: row.id,
